(function(){
  ...
  if(!canvas) return;
  const introStage = canvas.closest('.intro-stage');

  // safety mode: disable heavy intro on local dev or reduced motion
  if(disableIntro || prefersReduced){
  ...
  function scheduleNext(nowMs){
    ...
    if(nextMode === mode && Math.random() < 0.65) nextMode = (mode === 'logo') ? 'key' : 'logo';
    ...
    // quick but readable
    tTransDur = prefersReduced ? 0.50 : (0.28 + Math.random()*0.22);
  }
